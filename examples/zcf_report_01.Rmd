...
```{r fig1, fig.cap='Proportion of households with solar installations by wealth decile. 1 is the lowest wealth-decile,  10 is the highest. Data sourced from ABS 2017–18 Survey of Income and Housing, [@BEST2021106890].', echo=FALSE }

library(ggplot2)
data.file<- "data/solar_deciles.feather"
sd.df <-arrow::read_feather(data.file)
...
```{r code, fig.cap='In regions where domestic solar output now regularly exceeds local demand, such as Morphet Vale east in South Australia,  flow reversals on DNSP zone substations is adding costs to grid operation', echo=FALSE}
library(ggplot2)
library(nord)
